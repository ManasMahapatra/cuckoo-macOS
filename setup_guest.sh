...
agenturl="https://raw.githubusercontent.com/cuckoosandbox/cuckoo/master/cuckoo/data/agent/agent.py"
...
function welcome() {
printf '
   Semi-automatic installation and configuration of macOS VM for Cuckoo
-------------------------------------------------------------------------------

This script installs only open-source software and unmodified Apple binaries.
Original Script: https://github.com/img2tab/macos-guest-virtualbox

The script checks for dependencies and will prompt to install them if unmet and will guide you
through the steps needed to be followed to properly setup Cuckoo guest machine.
Some stages may fail due to errant keyboard presses; run the script with
"'${whiteonblack}"${0}"' stages stage_name '${defaultcolor}'" to see how to run only certain stages,
if you have already completed the previous ones.

'${whiteonblack}"Be very careful while pressing enter when the script asks you to. 
Kindly be patient for the terminal to promt before pressing the enter key when asked
by the script.\n\n"

printf "${defaultcolor}'The installation requires ${whiteonred}'37.5GB'${defaultcolor} of available storage,
...
7) Close the terminal. Reboot the VM, ensure that it auto-logins and check if sudo asks for password.
   If it is working fine and the system is ready plese press Enter.
'
}
